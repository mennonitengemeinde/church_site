...
class EventsView(View):
    ...
    def get(self, request, *args, **kwargs):
        context = {
            "current_church": (
                request.GET.get("church") if request.GET.get("church") else None
            ),
            "events": selectors.get_events_formatted_by_date(
                church_name=(
                    request.GET.get("church")
                    if request.GET.get("church") != "all"
                    else None
                )
            ),
...
class EventsAdminListView(PermissionRequiredMixin, AdminListView):
    ...
    def get_queryset(self):
        return selectors.get_admin_events(
            user=self.request.user, current_events_only=True, order_by_start="asc"
        )
...
class EventsAdminCreateView(PermissionRequiredMixin, BaseCreateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super(EventsAdminCreateView, self).get_context_data(
            object_list=object_list, **kwargs
        )
        context["event_templates"] = EventTemplate.objects.all()
        context["selected_template"] = (
            int(self.request.GET.get("template"))
...
class AttendantCreateView(SuccessMessageMixin, BaseCreateView):
    ...
    def get_success_message(self, cleaned_data):
        if cleaned_data.get("amount") > 1:
            ...
            people = "person"
        return (
            f"{cleaned_data.get('full_name')}, you have successfully signup {cleaned_data.get('amount')} {people} "
...
class AttendantAdminUpdateView(PermissionRequiredMixin, BaseUpdateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["btn_back_href"] = reverse_lazy(
            "schedules:attendants-admin-detail",
            kwargs={"event": self.kwargs.get("event"), "pk": self.kwargs.get("pk")},
        )
        return context
...
class AttendantAdminDetailView(PermissionRequiredMixin, BaseDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["btn_back_href"] = reverse_lazy(
            "schedules:events-admin-detail", kwargs={"pk": self.kwargs.get("event")}
        )
        return context
