...
def get_filtered_sermon_list(churches: List[int], speakers: List[int]):
    if churches and not speakers:
        return Sermon.objects.filter(visible=True, event__church__in=churches).order_by(
            "-event"
        )
    elif not churches and speakers:
        return Sermon.objects.filter(visible=True, speakers__in=speakers).order_by(
            "-event"
        )
    elif churches and speakers:
        return Sermon.objects.filter(
            visible=True,
            event__church__in=churches,
            speakers__in=speakers,
        ).order_by("-event")
    else:
...
def get_filtered_sermons(church: str = None, speaker: str = None):
    if church and not speaker:
        return Sermon.objects.filter(
            visible=True, event__church__name__iexact=church.replace("-", " ")
        ).order_by("-event")
    elif not church and speaker:
        return Sermon.objects.filter(visible=True, speakers=int(speaker)).order_by(
            "-event"
        )
    elif church and speaker:
        return Sermon.objects.filter(
            visible=True,
            event__church__name__iexact=church.replace("-", " "),
            speakers=int(speaker),
        ).order_by("-event")
    else:
        return Sermon.objects.filter(visible=True).order_by("-event")
